...
const TopCoursesSection = () => {
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription for top courses updates
    const channel = supabase
      .channel('top-courses-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          console.log('Top courses updated, refreshing...');
          fetchTopCourses();
        }
      )
      .subscribe();
  ...
  const fetchTopCourses = async () => {
    try {
      // First try to fetch from top_courses table (admin-managed)
      ...
      if (topCoursesError && topCoursesError.code !== 'PGRST116') {
        console.error('Error fetching top courses:', topCoursesError);
      }

      if (topCoursesData && topCoursesData.length > 0) {
        console.log('Fetched top courses from database:', topCoursesData.length);
        // Convert database format to component format
        const formattedCourses = topCoursesData.map(course => ({
          id: course.id,
          title: course.title,
          instructor: course.instructor || 'Expert Instructor',
          rating: course.rating || 4.8,
          reviews: course.review_count || 1000,
          students: course.student_count || 5000,
          price: course.price || 89.99,
          originalPrice: course.original_price || 199.99,
          duration: course.duration || '40 hours',
          lectures: course.lecture_count || 100,
          level: course.level || 'Intermediate',
          image: course.image_url || 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
          tags: course.tags || ['Cybersecurity'],
          lastUpdated: course.last_updated || 'January 2024',
          bestseller: course.is_bestseller || false,
          hot: course.is_hot || false,
          new: course.is_new || false
        }));
        setTopCourses(formattedCourses);
      } else {
        console.log('No admin-managed courses found, using defaults');
        // Fallback to default courses if no admin-managed courses exist
        setTopCourses(getDefaultTopCourses());
      }
    } catch (error) {
      console.error('Error in fetchTopCourses:', error);
      setTopCourses(getDefaultTopCourses());
    } finally {
      setLoading(false);
    }
  ...
  const getDefaultTopCourses = (): TopCourse[] => [
    {
      id: "1",
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Master technology skills with our featured courses designed by professionals for real-world application.
          </p>
        </motion.div>
                ...
                <div className="relative">
                  <div className="aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
                      <motion.div 
                        className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
